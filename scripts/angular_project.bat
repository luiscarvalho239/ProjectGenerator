...
:main
IF EXIST "%dirproj%\AngularApp" (
	cls
    echo O projeto %dirproj%\AngularApp já existe.
    goto :run_proj
) ELSE (
    cls
    echo %titleproj%
...
npm install -g @angular/cli && cd "%dirproj%" && ng new "AngularApp"
...
if /I "%isrunproj%"=="true" (
    ...
    cd %dirproj%\AngularApp && ng serve -o
) else (
    goto :exit_prog
)
