...
:main
IF EXIST "%dirproj%\vuejsapp" (
	cls
    echo O projeto %dirproj%\vuejsapp já existe.
    goto :run_proj
) ELSE (
    cls
    echo %titleproj%
...
npm install -g @vue/cli && cd "%dirproj%" && vue create "vuejsapp"

if %ErrorLevel% EQU 0 (
    echo.
    echo Gerou o projeto %dirproj%\vuejsapp com sucesso!
) else (
    echo.
...
if /I "%isrunproj%"=="true" (
    ...
    cd %dirproj%\vuejsapp && npm run serve
) else (
    goto :exit_prog
)
